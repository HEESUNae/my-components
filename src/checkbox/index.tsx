...
export function Checkbox({label, checked, disabled, ...rest}: CheckboxProps) {
    const [isChecked, setIsChecked] = useState<boolean>(!!checked); // 디폴트 선택여부
    ...
    return (
        <div>
            <input type="checkbox" id={id} onChange={handleCheckIcon} checked={isChecked}
                   disabled={!!disabled} {...rest}/>
            <label htmlFor={id}>
                {disabled ? <CheckDisabledIcon/> : isChecked ? <CheckOnIcon/> : <CheckOffIcon/>}
                {label && <p className={`${disabled && "disabled"}`}>{label}</p>}
            </label>
        </div>
    )
}
...
// 체크박스 disabled 예시 아이콘
function CheckDisabledIcon() {
    return (
